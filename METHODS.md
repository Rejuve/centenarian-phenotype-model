...
2. **Tier 2 (application):** a **standalone 32-item mobile survey** — 19 NHANES-aligned behavioral questions (re-asked at greater depth than the teaser, mapped to the top-ranked centenarian features, incl. a subjective-loneliness item corroborated by Holt-Lunstad 2015) plus 13 self-report clinical/health items. Free application tier; re-captures every variable fresh (no carry-over from Tier 1). (`tier2_model.yaml`)
...
| 2026-06-03 | Deployable scoring package overhaul (v0.2.0): Naive Bayes posterior layer, strict validation, completeness split, raw-value mappers, clock registry, phenotype domains, relative-longevity context (HMD baselines) + held-out NHANES 1999–2016 mortality calibration; CI matrix/lint/wheel/deterministic-artifact; stale docs removed. |
| 2026-06-03 | Tier 2 → 32 questions: added subjective-loneliness item (`q_loneliness`, social_connectedness, weight 0.40) with new `meta_analytic` basis, corroborated by Holt-Lunstad 2015 (isolation/loneliness OR 1.29/1.26). Curated cited aggregate evidence (`docs/EVIDENCE_LONGEVITY_FACTORS.md`); AI/LLM-use access screen (`docs/DATA_STRATEGY.md`). |
